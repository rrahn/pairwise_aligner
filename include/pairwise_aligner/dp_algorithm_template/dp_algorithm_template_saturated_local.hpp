...
public:
    ...
    {
        return _dp_vector.to_regular_score(score);
    }
...
protected:
    ...
    auto run(sequence1_t && sequence1, sequence2_t && sequence2, dp_column_t dp_column, dp_row_t dp_row) const
    {
        ...
        // In each block we track the relative optimal score.
        // After each block the relative optimal score must be converted to a global score.
        // The global score of each block is tracked for the maximum!
        ...
        for (size_t j = 0; j < row_block_count; ++j) {
            ...
            for (size_t i = 0; i < column_block_count; ++i) {
                ...
                // Check if both vectors are local at the same time!
                assert([&] ()
                {
                    auto m_row = current_row_vector.is_local();
                    auto m_col = dp_column_blocks[i].is_local();
                ...
                tracker.reset_block_max_score();

                base_t::compute_block(seq1_blocked[i],
                                      transformed_seq2,
                                      dp_column_blocks[i],
                                      current_row_vector,
                                      scorer.block_scheme(current_row_vector.is_local()),
                                      tracker);

                tracker.track(dp_column_blocks[i].to_regular_score(tracker.block_max_score()));
            }
