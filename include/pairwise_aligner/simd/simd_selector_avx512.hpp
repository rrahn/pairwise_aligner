...
template <typename simd_offset_t, size_t operand_count>
    requires (operand_count > 64 && operand_count <= 128)
struct simd_selector<simd_offset_t, selector_tag<operand_count, 8, 512>>
{
    static constexpr bool in_lane_shuffle = false;
...
// Set of functor classes for all kind of packing strategies?

template <typename simd_value_t, typename simd_offset_t, size_t size_v>
struct _simd_selector_avx512
{
    class type;
};

template <typename simd_value_t, typename simd_offset_t, size_t size_v>
using simd_selector_avx512 = typename _simd_selector_avx512<simd_value_t, simd_offset_t, size_v>::type;

template <typename simd_value_t, typename simd_offset_t, size_t size_v>
class _simd_selector_avx512<simd_value_t, simd_offset_t, size_v>::type
{
private:
    ...
    using selector_impl_t = simd_selector<simd_offset_t, selector_tag<size_v, sizeof(offset_t) * 8, max_simd_size * 8>>;
...
public:
    ...
    static constexpr address_t load(data_slice_t && data_slice) noexcept
    {
        ...
        assert(std::ranges::distance(data_slice) <= static_cast<std::ptrdiff_t>(selector_impl_t::max_operand_count));
...
private:
    ...
    static constexpr __m512i const & to_native(packed_t const & packed) noexcept
    {
