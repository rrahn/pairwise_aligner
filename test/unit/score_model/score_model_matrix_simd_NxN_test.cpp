...
TYPED_TEST(score_model_matrix_simd_test, access_same)
{
    ...
    // using simd_16_t = pa::simd_score<int16_t>;
    // auto to_native = [] (auto const & packed) -> __m512i const & { return reinterpret_cast<__m512i const &>(packed); };
    // auto to_packed = [&] (__m512i const & native) -> simd_index_t const & { return reinterpret_cast<simd_index_t const &>(native); };
    // auto to_packed_16 = [&] (__m512i const & native) -> simd_16_t const & { return reinterpret_cast<simd_16_t const &>(native); };

    // simd_index_t a{};
    // simd_index_t b{};
    // std::ranges::for_each(std::views::iota(0ull, simd_index_t::size), [&] (scalar_score_t j) {
    //     a[j] = j;
    //     b[j] = j + 64;
    // });
    ...
    // simd_index_t mod_idx{}; // all 0
    // __m512i idx_32x16_lo = _mm512_unpacklo_epi8(to_native(mod_idx), __m512i{});  // unpack and extend with zero.
    // __m512i idx_32x16_hi = _mm512_unpackhi_epi8(to_native(mod_idx), __m512i{});  // unpack and extend with zero.

    // std::cout << "idx_32x16_lo = " << to_packed(idx_32x16_lo) << "\n";
    // std::cout << "idx_32x16_hi = " << to_packed(idx_32x16_hi) << "\n";

    // // index_t idx_32x16_lo_packed = to_packed(idx_32x16_lo);
    // // index_t idx_32x16_hi_packed = to_packed(idx_32x16_hi);

    // // Load the scores in 2x8bit packs from the a and b register.
    // __m512i tmp_lo = _mm512_permutex2var_epi16(to_native(a), idx_32x16_lo, to_native(b));
    // __m512i tmp_hi = _mm512_permutex2var_epi16(to_native(a), idx_32x16_hi, to_native(b));

    // std::cout << "tmp_lo = " << to_packed(tmp_lo) << "\n";
    // std::cout << "tmp_lo = " << to_packed(tmp_lo) << "\n";
    // std::cout << "tmp_hi_16 = " << to_packed_16(tmp_hi) << "\n";
    // std::cout << "tmp_hi_16 = " << to_packed_16(tmp_hi) << "\n";

    // __mmask64 uneven_mask{};
    // tmp_lo = _mm512_mask_srli_epi16(tmp_lo, uneven_mask, tmp_lo, 8); // shift 8 bits right if mask was bit was 1.
    // tmp_hi = _mm512_mask_srli_epi16(tmp_hi, uneven_mask >> 5, tmp_hi, 8); // shift 8 bits right if mask was bit was 1.

    // std::cout << "tmp_lo = " << to_packed(tmp_lo) << "\n";
    // std::cout << "tmp_hi = " << to_packed(tmp_hi) << "\n";
    // std::cout << "tmp_hi_16 = " << to_packed_16(tmp_lo) << "\n";
    // std::cout << "tmp_hi_16 = " << to_packed_16(tmp_hi) << "\n";
    // simd_16_t mask{(int16_t)0x00FF};
    // __m512i packed = _mm512_packus_epi16(tmp_lo & to_native(mask), tmp_hi & to_native(mask));
    // std::cout <<s "packed = " << to_packed(packed) << "\n";
    ...
    // size_t c = 9;
    {
        ...
        std::array<simd_index_t, TestFixture::strip_width_v> symbols{};
